...
class DataFusion:
    ...
    def extract_physical_data(self, os_number, features=['Time', 'LL_dnp3_src', 'LL_dnp3_dst', 'LL_dnp3_len', 'LL_dnp3_ctl',
    ...
    def extract_physical_data_with_values(self, to_monitor, features=['Time', 'LL_dnp3_src', 'LL_dnp3_dst', 'LL_dnp3_len', 'LL_dnp3_ctl','TL_dnp3_tr_ctl', 'AL_dnp3_al_func', 'AL_dnp3_al_ctl', 'AL_dnp3_obj','DNP3 Object Count', 'DNP3 Objects', 'AL_Payload']):
        ...
        for os_key in to_monitor.keys():
            ptable_os = self.extract_physical_data(os_key)
    ...
    def process_snort(self,file_name):
        ...
        for alert in alerts[:-1]:
            ...
            unix_time = datetime.strptime('20/' + time, '%y/%m/%d-%H:%M:%S.%f').timestamp()
    ...
    def merge_snort(self):
        # found a bug on the snort time.. they are in texas time while rest follow california time
        for index, row in self.snort_table.iterrows():
            self.snort_table.loc[index,'Time'] = row['Time'] - 7200
