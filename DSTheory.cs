...
namespace DataFusionApp
{
    class DSTheory
    {
        public class MassFunction
        {
            ...
            public Dictionary<string, double> combine(Dictionary<string, double> mf1, Dictionary<string, double> mf2, bool normalization, int sample_count, bool importance_sampling, bool isDisjunctive, bool isCautious)
            {
                Dictionary<string, double> combined = new Dictionary<string, double>();
                if (sample_count == 0)
                {
                    combined = combinedDeterministically(mf1, mf2, isDisjunctive, isCautious);
                }
            ...
            public Dictionary<string, double> combinedDeterministically(Dictionary<string, double> mf1, Dictionary<string, double> mf2, bool isDisjunctive, bool isCautious)
            {
                Dictionary<string, double> combined = new Dictionary<string, double>();
        ...
        Artificial Intelligence 172, 234-264.*/
            public Dictionary<string, double> combineCautious(Dictionary<string, double> mf1, Dictionary<string, double> mf2, bool disjunctive)
            {
                ...
                //Dt 22nd December , still got to fix this refering the paper. 
                foreach (var item in wmin.Keys.Reverse())
                {
                    ...
                    if (disjunctive)
                    {
                        m.hyp_value = combine(m.hyp_value, m_simple.hyp_value, false, 0, false, true, true);
                    }
                    else
                    {
                        m.hyp_value = combine(m.hyp_value, m_simple.hyp_value, false, 0, false, false, true);
                    }
            ...
            public Dictionary<string, double> weight_function()
            {
                ...
                for (int i = 0; i < theta.Count; i++)
                {
                    ...
                    if (theta[i].Length < maxLen)
                    {
                        ...
                        //if (q_odd == 0) weights[theta[i]] = q_even;
                        //else if (q_even == 0) weights[theta[i]] = q_odd;
            ...
            {
                var combined = combine(m1.hyp_value, m2.hyp_value, false, sample_count, false, false, false);
